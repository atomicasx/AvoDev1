...
class fyn_QR_CODE_BOXESViewList extends ViewList
{
    /**
     * @var string $type
     */
    ...
    public function display()
    {
        ...
        if (!$this->bean || !$this->bean->ACLAccess('list')) {
            ...
            if(!empty($_REQUEST['parentid']) and ($_REQUEST['parent'] == "fyn_Stock_In")){

                $query = "select * from fyn_stock_in_fyn_qr_code_boxes_1_c where fyn_stock_in_fyn_qr_code_boxes_1fyn_stock_in_ida = '".$_REQUEST['parentid']."' and deleted = '0    '";
                $result = $db->query($query);
                $num = $result->num_rows;
                if($num > 0) {
                    $qrIds = [];
                    while ($row = $db->fetchByAssoc($result)) {
                        $qrIds[] = $row['fyn_stock_in_fyn_qr_code_boxes_1fyn_qr_code_boxes_idb'];
                        
                    }
                    if($qrIds) {
                        $qrIdstr = implode("','", $qrIds);
                    }
                    
                    $this->where = "fyn_qr_code_boxes.id in ('".$qrIdstr."')";
                    //echo $this->where;
                }
...
<a class="menuItem" style="width: 150px;" href="#" onmouseover='hiliteItem(this,"yes");'
        ...
        if(sugarListView.get_checks_count() < 1) {
            app.alert.show('custom-message-id',
                                {
                                    level: 'fail',
                                    messages: 'Please Select any record!',
                                    autoClose: true
                                });
            return false;
        }
        document.MassUpdate.action.value='displaypassedids';
        //document.MassUpdate.action.value='showPopup';
