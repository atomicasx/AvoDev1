...
if (!isset($_REQUEST['invoiceIds']) || empty($_REQUEST['invoiceIds']) || !isset($_REQUEST['templateID']) || empty($_REQUEST['templateID'])) {
...
global $mod_strings, $sugar_config;
    ...
    $countInvoice = $_POST['countInvoice'];
    $invoices = explode(",", $_POST['invoiceIds']);
    ...
    if($countInvoice > 0) 
    {
        ...
        {
            $bean = BeanFactory::getBean($_REQUEST['module'], $invoice);
            ...
            $text = str_replace("\$print_name_c", "\$" . $variableName . "_print_name_c", $text);
            $text = str_replace("\$signature_c", "\$" . $variableName . "_signature_c", $text);
            $text = str_replace("\$driver_c", "\$" . $variableName . "_driver_c", $text);
            $text = populate_group_lines($text, $lineItemsGroups, $lineItems);

            $converted = templateParser::parse_template($text, $object_arr);
            $headerData[] = templateParser::parse_template($header, $object_arr);
            $footerData[] = templateParser::parse_template($footer, $object_arr);

            $printableData[] = str_replace("\n", "<br />", $converted);
        }
...
if ($task == 'pdf' || $task == 'emailpdf') {
    ...
    try {
        ...
        $countPrintableData = count($printableData);

        if($countPrintableData > 0)
        {
            for($i = 0; $i < $countPrintableData; $i++) 
            {
                if($i!=0)
                {
                    $pdf->AddPage();
                }
                $pdf->SetHTMLHeader($headerData[$i]);
                $pdf->SetHTMLFooter($footerData[$i]);
                $pdf->WriteHTML($printableData[$i]);
