    ...
    public function updateProductQuantityBefore($bean, $event, $arguments)
    {
        if($_POST['module'] == "AOS_Invoices") {
            ...
            while ($row = $db->fetchByAssoc($result)) {
                ...
                $fetchInvoiceStockSQl = "select invoice_stock_c from aos_products_cstm where id_c = '".$row['product_id']."'";
                $resultInvoiceStockSQl = $db->query($fetchInvoiceStockSQl);
                $invoice_stock_c = 0;
                while ($rowInvoiceStockSQl = $db->fetchByAssoc($resultInvoiceStockSQl)) {
                    if((int)$rowInvoiceStockSQl['invoice_stock_c'] > 0) {
                    $invoice_stock_c = (int)$rowInvoiceStockSQl['invoice_stock_c'] - (int)$row['product_qty'];    
                }
                }
                ...
                fwrite($fp, " invoice_stock_c ".(int)$invoice_stock_c."\n\n\n");
                fwrite($fp, " product_qty ".(int)$row['product_qty']."\n\n\n");
