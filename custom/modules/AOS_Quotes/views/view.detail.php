...
class AOS_QuotesViewDetail extends ViewDetail
{
    ...
    public function display()
    {
		...
		if($this->bean->billing_account_id){
			$account_id = $this->bean->billing_account_id;
			$sql = "SELECT * FROM aos_invoices WHERE deleted=0 AND billing_account_id='$account_id' ";
			$result = $db->query($sql);
			/* echo '<pre>';
			print_r($result);
			die(); */
			//$OVERDUE_INVOICES = '<a href="index.php?module=AOS_Quotes&action=getBalanceInvoices&account_id='.$account_id.";
			$this->ss->assign('OVERDUE_INVOICES',0);
			$this->ss->assign('ACCOUNT_ID',$account_id);
			if($result->num_rows > 0){
				$this->ss->assign('OVERDUE_INVOICES',1);
			}
		}
