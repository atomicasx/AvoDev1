...
class fyn_routesViewDetail extends ViewDetail 
{
    ...
    function display()
    {
      ...
      <?php
		$this->ss->assign('GENROUTE', '<a target="_blank" id = "GenRoute" class="btn btn-default">Gen Route</a>');
		parent::display();
		
      ?>
      
      <?php 
		if($current_user->user_role_c != 'driverlocation'){	  
		
		?>
        
		<input type="button" name="select" value="Select" onclick="window.open('<?php echo $sugar_config['site_url'];?>/index.php?module=AOS_Invoices&action=invoices&route=<?php echo $this->bean->id;?>','Invoice List','height=800,width=850');" style="display:none">
            ...
            //if($numRows > 0) {
                ...
                //while($row = $db->fetchByAssoc($res)) {
                    ...
                    foreach ($orders as $key => $order){
						if($current_user->user_role_c == 'driverlocation' && $order->driver_c != $current_user->name){
							unset($orders[$key]);
						}
					}
					/* echo "<pre>";
                    ...
                    foreach ($orders as $order) {
					  ...
					  //if($order->driver_c == $current_user->name){
                        $sql = "select users.id, CONCAT(users.first_name, ' ', users.last_name) as driver_name 
                            from 
                            users 
                                    inner join users_cstm 
                                        on users.id = users_cstm.id_c
                                    inner join route_invoice_driver 
                                        on users.id = route_invoice_driver.driver
                            where  route_invoice_driver.invoice = '".$order->id."'
                                    and users.deleted = '0' and users.is_admin = '0' and users_cstm.user_role_c='driverlocation'";
                        $res = $db->query($sql);
                    ...
                    <tr>
                        ...
                        <td><a target="_blank" href="index.php?module=AOS_Invoices&record=<?php echo $order->id;?>&action=DetailView"><?php echo $order->number;?></a></td>
                        <td><?php
						...
						<td><?php echo $order->due_dates_c;?></td>
                    </tr>
                    <?php
					// }
				}
