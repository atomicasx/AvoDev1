...
class qr_QR_PalleteController extends SugarController{
     ...
     */
    function qr_QR_PalleteController(){
...
//require('WriteHTML.php');
        ...
        //	echo $_REQUEST['uid'];
            ...
            foreach ( $recordIds as $recordId ) 
            {
                ...
                //echo $recordId;
            	$pallete= BeanFactory::getBean('qr_QR_Pallete',$recordId);
                // echo "<pre>";print_r($pallete);
               // echo $pallete->qr_image_c;exit;
            	//echo 
            	if($pallete->qr_image_c == '')
            		continue;
            	$str='qrboximages/'.$pallete->qr_image_c.'.png';
            	$qr_id = $pallete->name; 
            	$no_of_boxes = $pallete->total_boxes_c;
            	$date = $pallete->date_entered;
            	$truck = $pallete->vehicle_number_c;
            	$farm = $pallete->farm_c;
					$pdf->SetFont('Arial','',10);
					//$m = $qr_id.$product_name;
				$pdf->Cell(70, 70, $pdf->Image($str,$pdf->GetX(),$pdf->GetY(),70,70), 0, 2, 'C',false);
                $pdf->Cell(70, 4, $qr_id, 0, 2, 'C',false);
                $pdf->Cell(70, 4, $no_of_boxes, 0, 2, 'C',false);
                $pdf->Cell(70, 4, $date, 0, 2, 'C',false);
                $pdf->Cell(70, 4, $truck, 0, 2, 'C',false);
