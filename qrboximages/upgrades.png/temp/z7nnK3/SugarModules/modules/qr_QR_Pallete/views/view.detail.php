...
class qr_QR_PalleteViewDetail extends ViewDetail 
{
	...
	function populatePalleteTemplates()
	{
		global $db,$app_list_strings; 
		$sql = "SELECT id, name FROM aos_pdf_templates WHERE deleted = 0 AND type='qr_QR_Pallete' AND active = 1"; 
	...
	{
		global $db, $app_list_strings,$app_strings, $mod_strings;
		...
		if($templates)
		{
			...
			foreach($templates as $template)
			{
				...
				$js = "document.getElementById('popupDivBack_ara').style.display='none';
				document.getElementById('popupDiv_ara').style.display='none';
				var form=document.getElementById('popupForm');
				if(form!=null){form.templateID.value='".$template."';
