...
class OutcomeReportDashlet extends DashletGenericChart
{
 ...
 */
        ...
        {
            return "<h3 class='noGraphDataPoints'>$this->noDataMessage</h3> <b>Akba</b>";
        }
...
EOD;
    ...
    protected function constructQuery()
    {
		 ...
		 */
		$query = "SELECT status,".
            db_convert('aos_invoices.invoice_date','date_format',array("'%Y-%m'"),array("'YYYY-MM'"))." as m, ".
            " sum(total_amount) as total, count(*) as opp_count FROM aos_invoices ";
		$query .= " WHERE aos_invoices.deleted=0 ";	
		/* if($this->obm_date_start && $this->obm_date_end){
			$query .= " AND aos_invoices.invoice_date >= ".db_convert("'".$this->obm_date_start."'",'date') ." AND aos_invoices.invoice_date <= ".db_convert("'".$this->obm_date_end."'",'date');
		}else{
			$query .= " AND aos_invoices.invoice_date >= DATE_SUB(CURDATE(),INTERVAL 30 DAY) AND aos_invoices.invoice_date <= CURDATE() ";
		} */
		$query .= " AND aos_invoices.invoice_date >= DATE_SUB(CURDATE(),INTERVAL 30 DAY) AND aos_invoices.invoice_date <= CURDATE() ";
        /* if (count($this->obm_ids) > 0)
            $query .= " AND aos_invoices.assigned_user_id IN ('" . implode("','",$this->obm_ids) . "')"; */
		
		
		
        $query .= " GROUP BY status,".
            db_convert('aos_invoices.invoice_date','date_format',array("'%Y-%m'"),array("'YYYY-MM'")) .
            " ORDER BY m";
    ...
    protected function prepareChartData($data,$currency_symbol, $thousands_symbol)
    {
        ...
        foreach($data as $i)
        {
            $key = $i["m"];
            $status = $i["status"];
            $invoice_status_option = $i["invoice_status_dom"];
            if(!in_array($key,$chart['labels']))
            {
                $chart['labels'][] = $key;
                $chart['data'][] = array();
            }
            if(!in_array($status,$chart['key']))
                $chart['key'][] = $status;

            $formattedFloat = (float)number_format((float)$i["total"], 2, '.', '');
            $chart['data'][count($chart['data'])-1][] = $formattedFloat;
            $chart['tooltips'][]="<div><input type='hidden' class='status' value='$invoice_status_option'><input type='hidden' class='date' value='$key'></div>".$status.'('.$currency_symbol.$formattedFloat.$thousands_symbol.') '.$key;
        }
        return $chart;
    }
}