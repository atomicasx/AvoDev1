...
function display_lines($focus, $field, $value, $view)
{
    ...
    if ($view == 'EditView') {
        ...
        $html .= '<input type="hidden" name="vathidden" id="vathidden" value="'.get_select_options_with_id($app_list_strings['vat_list'], '').'">
            <input type="hidden" name="productcategorylisthidden" id="productcategorylisthidden" value="'.get_select_options_with_id($app_list_strings['product_category_list'], '').'">
        ...
        if ($focus->id != '') {
            ...
            $sql = "SELECT pc.verdes_firm_price_c, pc.rayados_breaking_price_c, pc.maduros_ripe_price_c, pg.id, pg.group_id FROM aos_products_quotes pg inner join aos_products_cstm pc on pg.product_id = pc.id_c LEFT JOIN aos_line_item_groups lig ON pg.group_id = lig.id WHERE pg.parent_type = '" . $focus->object_name . "' AND pg.parent_id = '" . $focus->id . "' AND pg.deleted = 0 ORDER BY lig.number ASC, pg.number ASC";
            ...
            while ($row = $focus->db->fetchByAssoc($result)) {
                ...
                $lineData = $line_item->toArray();
                $lineData = array_merge($lineData, array("verdes_firm_price_c"=>$row['verdes_firm_price_c'], "rayados_breaking_price_c" => $row['rayados_breaking_price_c'], "maduros_ripe_price_c" => $row['maduros_ripe_price_c']));
                $line_item = json_encode($lineData);
                //echo "<pre>";
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            ...
            if ($line_item->product_id != '0' && $line_item->product_id != null) {
                if ($productCount == 0) {
                    ...
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>Category</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".$line_item->product_category_list_c."</td>";
