...
class AOS_Products_Quotes extends AOS_Products_Quotes_sugar
{
    ...
    {
        global $db;
        $sqlQuote = "select * from aos_products_quotes inner join aos_products_quotes_cstm on aos_products_quotes.id = aos_products_quotes_cstm.id_c where id = '".$id."'";
        ...
        //$invoice_stock_c = 0;
	//if((int)$rowProduct['invoice_stock_c'] >= 0) {
        $invoice_stock_c = (int)$rowProduct['invoice_stock_c'] + (int)$newProdQty;
    //}	 
    $updateProductQty = "update aos_products_cstm set invoice_stock_c = '".$invoice_stock_c."'";
	if($rowQuote['product_category_list_c'] == "Verdes_Firm"){     
        
		$verdes_firm_c = (int)$rowProduct['verdes_firm_c'] - (int)$newProdQty;
		$updateProductQty.=", verdes_firm_c = '$verdes_firm_c'";
	} else if($rowQuote['product_category_list_c'] == "Rayados_Breaking"){
        $rayados_breaking_c = (int)$rowProduct['rayados_breaking_c'] - (int)$newProdQty;
		$updateProductQty.=", rayados_breaking_c = '$rayados_breaking_c'";
	} else if($rowQuote['product_category_list_c'] == "Maduros_Ripe"){
        $maduros_ripe_c = (int)$rowProduct['maduros_ripe_c'] - (int)$newProdQty;
		$updateProductQty.=", maduros_ripe_c = '$maduros_ripe_c'";
	}

        //$balance = (int)$rowProduct['balance_c'] - (int)$newProdQty;
        
        $updateProductQty.=" where id_c = '".$rowQuote['product_id']."'";

	$fp = fopen("updateProductQuantityINQuote.txt", "a");
            
	fwrite($fp, $updateProductQty."\n\n\n");
    fwrite($fp, "invoice_stock_c : ".(int)$rowProduct['invoice_stock_c']."\n\n\n");
    fwrite($fp, "newProdQty : ".(int)$newProdQty."\n\n\n");
